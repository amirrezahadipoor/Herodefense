 ...
 *
 * <p>Every entry here carries a number or a name that the caller has already formatted: a gain reads
 * "+30" ({@code GameNumbers.signed} exists for exactly that) and a coin amount is "$ 30". So the patterns
 * take strings, not longs, and the caller goes through {@code GameLocale.number} — the same reason
 * {@link Translated#text} refuses objects.
 *
...
public enum ItemStrings implements Translated {
    ...
    /** Auto-sell conversion pop-up, in the "+$ n" shape the combat renderer has always drawn;
     *  the pickup coin above reads "$ +n" in its own table entry — two pop-ups, two frozen strings. */
    FLOATING_COIN_AUTOSELL("+$ %1$s"),

    /** Anvil feedback: what happened, to what, and what it cost. */
    FORGE_REFORGED("REFORGED  |  %1$s  |  -$ %2$s"),
    FORGE_AFFIX_REROLLED("AFFIX REROLLED  |  %1$s  |  -$ %2$s"),
    FORGE_NEED_COINS("NEED $ %1$s MORE  |  ANVIL"),
    FORGE_NOT_FORGEABLE("ANVIL TAKES RARE & LEGENDARY ONLY"),
    FORGE_MAXED("FULLY REFORGED  |  %1$s"),

    /** The two equipment sets, named as a player reads them in the Inventory status line. */
    SET_VERDANT_COVENANT("Verdant Covenant"),
    SET_BASTION_OATH("Bastion Oath"),

    /** An item's name with its forge level on the end: "Starfall Bow +1". */
    ITEM_UPGRADE_LEVEL("%1$s +%2$s"),
     ...
     */
    SETS_STATUS("SETS: %1$s | %2$s"),
    SET_PROGRESS("%1$s %2$s/%3$s");

    private final String english;

    ItemStrings(String english) {
