 ...
 * The beats speak the milestone reflections' own entries -- the table holds Pip's words once,
 * and the breather gives them a stage instead of a banner. They play between waves, never
 * during combat, so the silence rule never hears them.
 */
public final class BreatherLines {
    ...
    public static StoryStrings entryFor(int waveNumber) {
        return switch (waveNumber) {
            case 25 -> StoryStrings.REFLECTION_WAVE_25;
            case 75 -> StoryStrings.REFLECTION_WAVE_75;
            case 125 -> StoryStrings.REFLECTION_WAVE_125;
            case 175 -> StoryStrings.REFLECTION_WAVE_175;
            default -> null;
        };
    }
