...
- [ ] **R7.3 Persian + RTL**: locale-aware string table, mirrored layout, and a test that fails on
  ...
  Two of the three are done and the third is half done, which is why this is still unticked.
  **The table** is `i18n/`: `GameLanguage`, `Translated`, `GameNumbers`, `GameLocale` and five per-screen enums
  ...
  (`1a3b379`). **The gate** is `DrawnStringProvenanceTest` (`f485b23`): a literal counts as user-facing when it
  sits in a `draw*` call, in a method whose name says it produces words, or in a formatter, and inside those spans
  it has to look like a sentence -- which is how diagnostics stay English on purpose. Nine renderers still draw
  their own words and are named in `NOT_YET_MIGRATED`, a ratchet checked in both directions:
  `CodexOverlayRenderer`, `FloatingCoinTextRenderer`, `InventoryOverlayRenderer`, `LevelUpOverlayRenderer`,
  `OnboardingOverlayRenderer`, `RewardCardOverlayRenderer`, `RootNetworkOverlayRenderer`,
  `StatShopOverlayRenderer`, `TrialDraftOverlayRenderer`. **Mirroring** exists and is proven on one screen
  ...
  Four of the five localised screens mirror (`e0b0146` settings, `bd9d4a5` menu, pause overlay and run summary),
  each of them checked against its own touch layout first: the menu's rows, the pause overlay's and all three
  game-over buttons are 120f over 480f, so every box is its own mirror image and `input` needed no change.
  What remains, in the order it has to happen: the HUD, whose five buttons are not symmetric (`SPEED_X` 430f,
  `PAUSE_X` 570f, `INVENTORY_X` 195f, `SHOP_X` 375f, `ULTIMATE_X` 555f over widths of 120f and 150f), so what it
  draws and `HudTouchLayout` have to change in one commit or every button ends up somewhere the finger is not;
  then the nine ratcheted renderers, one table each, taking a name off the list per commit.
...
1. **Repo rubric (Phase 97):** the nine in-scope categories score **≥ 900 of 920** — the release category
   ...
   **Measured at Phase 97: 722 of 920** (`docs/audit/AUDIT_2026-09-17.md`, commit `12e4dc0`). The gate is not
   met, and the audit itemises the 198 missing points sub-item by sub-item: the largest are Persian and RTL
