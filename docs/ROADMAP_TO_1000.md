...
- [ ] **R7.3 Persian + RTL**: locale-aware string table, mirrored layout, and a test that fails on
  ...
  **The table** is `i18n/`: `GameLanguage`, `Translated`, `GameNumbers`, `GameLocale` and five per-screen enums
  ...
  What remains, in the order it has to happen: mirror the menu, the pause overlay and the run summary, whose tap
  boxes are all symmetric (`BUTTON_X` 120f + `BUTTON_WIDTH` 480f = 720f, and the same 120f/480f for all three
  game-over buttons), so their visuals can mirror without touching `input`; then the HUD, which cannot, because
  its five buttons are not symmetric (`SPEED_X` 430f, `PAUSE_X` 570f, `INVENTORY_X` 195f, `SHOP_X` 375f,
  `ULTIMATE_X` 555f over widths of 120f and 150f) and mirroring what it draws without mirroring
  `HudTouchLayout` would put every button somewhere the finger is not; then the nine ratcheted renderers, one
  table each, taking a name off the list per commit.
