...
- [ ] **R7.3 Persian + RTL**: locale-aware string table, mirrored layout, and a test that fails on
  ...
  **The gate** is `DrawnStringProvenanceTest` (`f485b23`, widened to all 257 main sources by `b1b5d41`): a
  ...
  directions: a name that stays after its file is migrated fails the build too. It is down to **17 files**,
  roughly 250 strings: `AffixId`, `BossTitleCards`, `CeremonyLines`, `CodexOverlayRenderer`, `Epilogue`,
  `InventoryOverlayRenderer`, `LevelUpOverlayRenderer`, `MythicEffects`, `ReflectionLines`, `RewardCardId`,
  `RewardCardOverlayRenderer`, `SkillEvolution`, `SkillId`, `StatShopOverlayRenderer`, `StatTooltips`,
  `Trophy`, `TrophyPresenter`.
