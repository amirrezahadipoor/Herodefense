...
- [ ] **R4.6 The second half's shallow step.** Waves 101-150 are only **4.7%** heavier than waves 51-100 while the
  step before them is **71%**: the run does rise after wave 40 (measured, and now gated by
  `WavePressureCurveTest`), but the shallowest point of the curve is the transition into the second half, where
  `SECOND_HALF_HEALTH_GROWTH` drops to 1.023 against the base 1.037 and the middle 1.041. Two repairs were measured
  and both were rejected on the shipped ceilings — from wave 121, `1.025/1.0085` lifted the third quarter to x1.092
  but pushed a bare-run spike to 0.3540 against 0.35 and a tier-10 STRENGTH median to 0.6765 against 0.60, and
  `1.024/1.008` (health only, no damage change) still broke trial pairs at 0.4071-0.4732 against 0.40. The numbers,
  the candidates and the reason each failed are in `docs/BALANCE.md`; closing this item means buying the headroom —
  a slower late damage rate, a trial ceiling raised with its reason written down, or an economy-side change — rather
  than spending it.
...
| 2026-09-17 | 91 | R4.2 | the boss telegraph asserted as a contract: five promises over four identities x forty encounters x tiers 0/3/6/10 x both vigils, 6 cases in 0.06 s, with the roster table and the per-strike damage published | `PENDING` |
