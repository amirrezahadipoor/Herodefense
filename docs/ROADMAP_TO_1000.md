...
- [x] **R5.2 Compose a runtime tier from the genuine masters.** The promotion is measured, the ceiling it needs
  ...
  hash ledger and the draw scale -- which shipped together, because none of them means anything alone.
  The composition is `tools/visual/compose_master_tier.py`, and it selects **by measurement**: the sheets
  `MASTER_TIER_PROVENANCE.md` classifies as independent renders *whose page is a 384 px frame grid*. That is
  eight sheets -- four regular enemies, three bosses and the hero -- and it is deliberately not every sheet the
  provenance table lists: the arena, the props, the ground tiles and the eight equipment icons among the 27 are
  already at the size they were rendered at, and the tree sheets and `void_knight` are classified *inconclusive*
  rather than independent, so they keep the reviewed tier rather than being promoted on a hope. The three
  health-potion masters are the one honest gap: they are independent renders at 192 px, they are icon cells the
  runtime draws at 96 px or less, and composing them would move the shipped catalog by 331,776 bytes while
  invalidating the accepted UI-assets review for a difference the drawing size cannot show. That decision is
  recorded as a decision rather than taken quietly, and it is exactly the 331,776 bytes between the projection
  above and the shipped catalog: **518,959,104 bytes measured**, so the arithmetic in the manifest reads
  518,959,104 + 6,040,896 = 525,000,000. The composition also doubled the page the sprites live on, so
  `maxAtlasPageSize` moved 2048 -> 4096 (the render's page is 3840x1536) and the residency budget moved with the
  pixels: the live combat set measures **207,765,504 bytes (198.1 MiB)** against a
  `decodedCombatResidencyBudgetBytes` the owner raised to **209,715,200 bytes** (200 MiB) on 2026-09-17, with the
  measured number, the cause (eight sheets at 384 px frames rather than 192 px) and the device's own budget
  unchanged at 400 MiB written into the manifest beside it -- a raised budget with its reason next to it, not a
  quiet one. The reviews keep their authority over the new tier: every batch review regenerates against the
  composed candidate, each composed key is bound to the hash of its shipped sheet and atlas, the hash ledger
  (`docs/asset_hashes.json`) is regenerated by the tool that composed the sheets so it cannot describe a
  different tree, and `:core:test` gains `ComposedTierDrawScaleTest`, which holds the two halves apart: the
  composed sheets are 384 px, every enemy is still drawn at the size the balance table gives it, and no renderer
  in the package may measure a region to decide how big to draw it. The draw scale needed no change because
  `CombatEntityRenderer` draws into the size it is handed -- and the test is what says so next time. CI now
  re-derives both halves on every push (`compose_master_tier.py --check`,
  `create_master_tier_review.py --check`), so a composed sheet that drifts from the master commit's own bytes, or
  from the review that accepted it, fails the build instead of landing.
...
- [ ] **R7.3 Persian + RTL**: locale-aware string table, mirrored layout, and a test that fails on
  ...
  Two of the three are done and the third is most of the way, which is why this is still unticked.
  **The table** is `i18n/`: `GameLanguage`, `Translated`, `GameNumbers`, `GameLocale` and ten per-screen enums
  listed in `GameStrings`, so the font derives its glyph set from the same list `TranslationTableTest` checks
  (`c441453`, `856fd2a`). Vazirmatn Bold and ExtraBold are committed beside Nunito under the SIL OFL, and
  `OverlayText.visual` routes every string through `PersianShaper` and `BidiReordering`, which reproduce all 245
  golden vectors that `arabic_reshaper` 3.0.1 and `python-bidi` 0.6.11 produce and CI re-derives from those pinned
  versions before the JVM starts (`54713a2`, `954c5d4`, `cd89d84`, `239f1b7`). Five screens read their words out
  of the tables and draw no literal: settings (`c165d20`), and the menu, pause overlay, run summary and HUD
  (`1a3b379`). Five more screens and four content enums joined them on 2026-09-18 -- the omens and the two run
  lengths (`73ce75a`), the first vigil's coaching (`fcf4347`), the anvil, the two equipment sets and the coin
  that floats off a kill (`8715cbe`), the Root Network screen (`dca2e4b`) and the Convergence draft with the
  thirteen trials themselves (`bf3776e`) -- so ten tables now hold 181 entries, and every number that reaches
  one of them goes through `GameLocale.number` or `.percent` first.
  **The gate** is `DrawnStringProvenanceTest` (`f485b23`, widened to all 257 main sources by `b1b5d41`): a
  literal counts as user-facing when it sits in a `draw*` call, in a method whose name says it produces words,
  in a formatter, or in an enum constant's arguments, and inside those spans it has to look like a sentence --
  which is how diagnostics stay English on purpose. Widening it found 320 untranslated strings in 27 files
  rather than the nine renderers it had been listing, and `NOT_YET_MIGRATED` is a ratchet checked in both
  directions: a name that stays after its file is migrated fails the build too. It is down to **13 files**:
  `AffixId`, `CodexOverlayRenderer`, `Epilogue`, `InventoryOverlayRenderer`, `LevelUpOverlayRenderer`,
  `MythicEffects`, `RewardCardId`, `RewardCardOverlayRenderer`, `SkillEvolution`, `SkillId`,
  `StatShopOverlayRenderer`, `StatTooltips`, `Trophy` -- the five remaining overlays and the eight content
  enums and catalogs behind them. The game's own voice left the list first (`StoryStrings`: the four boss title
  cards, the five ceremony beats, the six wave reflections and the trophy line), because those are the lines a
  player meets without opening a menu.
  The gate has one blind spot, named because it is load-bearing: its content rule reads *enum* constructor
  arguments, so `ascension/RootNetworkCatalog` (23 node names and their bonus lines, arguments to a static
  `node(...)` factory) and `items/EquipmentCatalog` (every item name, which reaches the anvil's feedback through
  `%1$s`) are not caught and are not translated. Extending the rule to static content factories is open work.
  **Mirroring** exists and is proven on one screen
  (`e0b0146`): `render/UiMirror` speaks leading and trailing, `OverlayText.drawLeading`/`drawTrailing` measure
  shaped before they mirror, the settings screen is placed entirely by insets, and its close button's tap target
  moves with the drawing through `SettingsTouchLayout.closeX()`.
  Five screens mirror (`e0b0146` settings, `bd9d4a5` menu, pause overlay and run summary, `dca2e4b` the Root
  Network's close box and its hit test), each checked against its own touch layout first: the menu's rows, the
  pause overlay's and all three game-over buttons are 120f over 480f, and the draft screen's panels are 600f at
  x=60f, so every one of those boxes is its own mirror image and needed no call. The coaching banner and the
  draft cards anchor their text to a container instead of to a number, which is what keeps a wider Persian line
  inside the band it is drawn in.
  What remains, in the order it has to happen: the **HUD**, which is the one screen that is not a set of insets
  inside symmetric containers -- its two info panels span 18f..414f of 720f, its health and experience bars are
  inset 73f/31f inside a panel that *is* symmetric, its five buttons are not (SPEED_X 430f, PAUSE_X 570f over
  120f; INVENTORY_X 195f, SHOP_X 375f, ULTIMATE_X 555f over 150f), and mirroring it means also reversing the
  direction both bars fill, since a progress bar that grows left-to-right in a right-to-left layout is a bar
  that empties as it fills. All of that has to land in one commit with `HudTouchLayout` or every button ends up
  somewhere the finger is not; it has not been attempted rather than attempted blind, because nothing in this
  sandbox can show the result. Then the 17 ratcheted files, one table each, taking a name off the list per
  commit, and the two catalogs the gate cannot see.
- [x] **R7.4 Back button** handled in game and menus, with an instrumentation test. The re-audit scored this
  ...
  -- the router over the real controllers and a host that answers the ten calls a press may make and refuses
  the other thirty-six, so the button's reach is a stated fact rather than a default, asserting that what a
  press does is what the matching button does (two presses to leave a codex entry, one to leave an empty
  shelf, a held choice making no calls at all); and `BackButtonNavigationTest` on the emulator, which injects a real `KEYCODE_BACK`
...
- [x] **R8.2 A memory budget enforced by a test.** `RuntimeResidency` computes residency from the
  ...
  composed fit -- they cost 518,959,104 of it) and the live combat set against
  `decodedCombatResidencyBudgetBytes`, a deliberate **200 MiB**, raised from 100 MiB in the same decision
  because the eight 384 px sheets took the live set to 198.1 MiB. The measured values are a logged run rather
  than a sentence: `perf:2026-09-17-composed-tier-residency` records the catalog at 518,959,104 bytes of the
  525,000,000 budget and the live combat set at 207,765,504 bytes -- **198.1 MiB** of the 200 MiB budget, with
  11.4 MiB of headroom. (This row used to say 95.9 MiB against a 100 MiB budget; that was the pre-composition
  arithmetic, and the logged run is what the file says now.) The test fails if either number moves, and `PremiumAssetContractTest`
...
| 2026-09-17 | — | Roadmap | Table B (the phase index for the experience phases 98-145) is deleted at the owner's direction; the experience rubric and the R9-R16 items it points at stay, and the numbered phases now end at 97 | `e2e994e` |
| 2026-09-17 | 92 · 93 | R5.2 · R5.7 | R5.7 closes: the three runtime effects (rarity glow, impacts, trails) with the tests that bind each one to its state and the emulator captures that show them under the brightness contract; R5.2 gets the measurement it was missing -- composing the runtime tier from the 27 genuine masters adds 134,418,432 bytes against 5,127,552 bytes of headroom, so the item states its gap in bytes instead of in adjectives | `c2d90ad` |
...
| 2026-09-17 | 96 · 93 | R8.1 (device) · R5.4 | the device failure was the test's own readback, and the pixels proved it: `ByteBuffer.array()` on a *direct* buffer hands back the allocation rather than the readback, seven bytes longer than the image, so every comparison this test had made was against a view that did not start at the picture; copying the bytes out by hand makes the emulator decode the container **pixel for pixel** -- run [`35213390481`](https://github.com/amirrezahadipoor/Herodefense/actions/runs/35213390481) (job `105176038768`) logs `maxDelta=0 exactFraction=1.000` with the uncompressed control at `(0,0)` and zero pixels beyond tolerance, so a real decoder difference still fails while a draw an emulator shifts cannot be charged to the encoder; the stage-grade contract passed on that run as well (dawn 24.872772/57.025925/43.373272 lit 0.6219 against hollow 24.276075/56.541542/43.506958 lit 0.6208, and 24.974325/57.135735/43.47092 lit 0.6226 against 24.274624/56.54204/43.507607 lit 0.6208 on the run R5.4 closed on) and it is that pair R5.4 stands on; the composed-tier check that had been dying inside `git show` now names a one-commit checkout as its cause with `fetch-depth: 0` behind it | `a455894` · `7230a30` |

| 2026-09-17 | 97 | Phase 97 | the re-audit is published with the method, the rubric and the numbers of the 2026-09-16 audit: `docs/audit/AUDIT_2026-09-17.md` scores **722 of 920 in scope** against the audited 505 in scope, from sub-item verdicts whose weights are printed, and it writes the gate's result as not met with the 198 missing points itemised in simple language; the figures are not typed -- `tools/audit/measure_round.py` reads the tree, a real `:core:test` run (179 classes, 720 tests, 0 failures, 36.8 s), the manifest, the hash ledger and the filed runs, and its output is frozen beside the audit as `docs/audit/MEASUREMENTS_2026-09-17.json` so the numbers can be re-taken and diffed. *(Corrected 2026-09-18: that audit's category rows sum to 731, not 722, and its own §2 and §8 say 731 -- the 722 was this row's and the README's, and the re-audit scores the difference in its category 10.)* The same push closes R8.1's device half on run [`35213390481`](https://github.com/amirrezahadipoor/Herodefense/actions/runs/35213390481) (job `105176038768`): `maxDelta=0 exactFraction=1.000`, control `(0,0)`, and both workflows green | `12e4dc0` · this push |
...
| 2026-09-17 | 95 | R7.4 (CI) | the first push of the Back button turned both jobs red, and both failures were the new code's own. `pmdTest` found four violations in the routing test's host, which was a `java.lang.reflect.Proxy` (`UseProperClassLoader`, `UseVarargs`, two `UnusedFormalParameter` -- the last two are the `InvocationHandler` signature itself, so no rule could be satisfied without silencing it): the proxy is gone and an explicit `FakeHost` answers the ten calls a press may make and *refuses* the other thirty-six, which asserts the button's reach instead of defaulting it; `spotbugsTest` then flagged the fake's never-written `gameOverSeconds`, now a named constant. The emulator job never reached a device: `androidx.test:core:1.6.1` has no `ActivityScenario.State` -- its `getState()` returns `androidx.lifecycle.Lifecycle.State`, read off the published AAR with `javap` -- and five references to it did not compile. The sandbox has no Android SDK, so `androidTest` sources are compiled by CI for the first time; that is now recorded here rather than discovered again | `b6c0e08` · this push |
| 2026-09-17 | 95 | R7.4 (CI) | the Back button's device test took three pushes to reach a device and every one of them died on a compile error rather than an assertion: `ActivityScenario.State`, which does not exist in `androidx.test:core:1.6.1` because `getState()` returns `androidx.lifecycle.Lifecycle.State`; JUnit 5's argument order in a JUnit 4 `assertEquals`, where the message comes first; and a touch counter reassigned between taps and captured by four lambdas, which Java refuses because a captured local has to be effectively final. The third is a rewrite rather than a patch -- `tap(...)` now takes the reading, taps, waits for the game to handle it and returns the correction, so a journey reads as one call per tap and no counter is shared across a lambda, and `openMenuAction` asks `MainMenuTouchLayout` which row carries the action instead of copying a y value, which is the rule R7.7 was written for. Three pushes were needed because the sandbox has no Android SDK, so `androidTest` sources were compiled by CI for the first time; there is now a local harness that compiles them against the real libGDX and JUnit jars, the real compiled core classes and hand-written stubs for only what the SDK and androidx.test provide, so the next device test is compiled before it is pushed rather than after. Both journeys pass on the API 35 emulator and the run is green end to end | `3b96420`, verified on `54713a2` (run [`35255737626`](https://github.com/amirrezahadipoor/Herodefense/actions/runs/35255737626)) |
| 2026-09-17 | 95 | R7.3 | the two pieces of evidence the shaper was written against, landed before it: Vazirmatn Bold and ExtraBold (123,036 and 123,324 bytes) with the full SIL OFL 1.1 text beside them in `android/assets/fonts/`, chosen because its alef, its ye and its kaf are drawn for Persian reading rather than borrowed from an Arabic font -- 2,048 units per em, 1,333 glyphs per face -- and `tools/i18n/make_shaping_vectors.py` with `golden/shaping-vectors.txt`: 245 cases, each a logical string and the exact display string it must become, produced by `arabic_reshaper` 3.0.1 and `python-bidi` 0.6.11 and re-derivable by the tool's `--check`. Font coverage was measured rather than assumed: all 171 distinct codepoints the vectors use are present in Vazirmatn-Bold, zero missing | `54713a2` |
| 2026-09-17 | 95 | R7.3 | the shaper. `render/PersianShaper.shape(String)` picks each letter's contextual form while the text is still in logical order -- because "the letter before it" means the one before it in the sentence, not the one to its left on screen -- and `render/BidiReordering`, a pass-by-pass port of `bidi.algorithm.get_display` from P2/P3 through L4, then puts the result in visual order and mirrors the brackets. All 245 golden vectors reproduce codepoint for codepoint, and the corpus earned its keep on the way there: the zero width joiner was asked whether it joins backwards through a table covering only U+0621..U+06D3, answered no, and silently stopped the join in every word that used one, so "می‌شود" lost its medial yeh -- `ب‍ب` was the vector that caught it, and not one I would have written by hand. Three lookup tables (77 letters over four forms, the 5 ligatures the default configuration enables in alternation order, 362 mirrored pairs) are generated out of the two libraries by `tools/i18n/make_shaping_tables.py`, and CI re-derives both the vectors and the tables from pinned versions before the JVM starts, so neither can drift into being stale evidence. Four static-analysis findings arrived with the slice and all four were fixed rather than excluded; `UseVarargs` is excluded with its reason in the ruleset and in `docs/STATIC_ANALYSIS.md`. Core 184 classes / 770 tests / 0 failures (was 183/758). Nothing renders Persian yet: the face is committed and the shaper is here, but `GameFonts` does not load Vazirmatn and `OverlayText` does not call `shape` | `954c5d4` |

| 2026-09-17 | 95 | R7.3 | the settings screen's language row, which is the first thing in the game that can choose a language: `GameSettings.language` is a `GameLanguage` rather than a `String` or an `int`, so an invalid language cannot be represented and `cycleLanguage()` steps it the way `cycleSoundVolume()` returns its new label. Language is a device-local preference like the volumes and not a property of a run, so `LocalSettingsRepository` owns it under `display.language`, `GameLanguage.fromCode` turns a hand-edited or future-version code into English at load rather than at draw time, and `load()` is where `GameLocale.use` happens -- loading the device's preferences is what makes the game speak them. `HeroDefenseGame` stays at its frozen 771 lines and 63 fields. `SettingsTouchLayout.LANGUAGE_ROW_Y` is 300f because that is the only free band on the screen: the level rows end at 450f..600f above it and the footer note panel occupied 140f..260f below it only after moving down from 250f, so the row clears both by a full `ROW_HEIGHT` of 150f. `SettingsOverlayRenderer` now contains no literal it draws, and two of its words are computed rather than stored: `nativeName()` answers a language in that language's own name, because a player who cannot read the screen is looking for a word they recognise and "Persian" written in English is not it. Core 191 classes / 805 tests / 0 failures (was 190/801), `ciStaticAnalysis` green | `c165d20` |
| 2026-09-17 | 95 | R7.3 (CI) | [!] four pushes went red and the failure was the evidence gate, not the code it was guarding. `unit-tests` died in "Re-derive the shaping vectors" on `c441453`, `ef03d00`, `856fd2a` and `cd89d84`, before a single Java test ran, with `DRIFT tools/i18n/golden/shaping-vectors.txt ... first difference at line 5`: same 253 lines, same 245 vectors, same two pinned libraries, and the only difference was the interpreter's patch level written into the header by the machine that generated the file (CPython 3.13.14) and compared byte for byte by a runner carrying 3.12.3. The gate was checking the runner instead of the evidence. A header that names the interpreter is only honest at the granularity that can matter: `python-bidi` asks `unicodedata` for each character's bidi class and its mirrored pair, and that table belongs to the CPython minor, not its patch -- 3.12 carries Unicode 15.0.0 and every 3.13.x carries 15.1.0. The header now records `CPython 3.13 with Unicode 15.1.0` and `test-core.yml` pins that minor with `actions/setup-python` immediately before the two i18n steps, after every other python gate in the job, so nothing else changes interpreter. Nothing was relaxed: `--check` still compares the whole file, both reference libraries are still pinned and asserted inside both scripts, and regenerating changed one comment line and no data line. Both jobs green from `a17f010` on | `239f1b7` |
| 2026-09-17 | 95 | R7.3 | the language test gives back the locale it borrowed. `GameLocale` is process-wide and Gradle runs two forks, so `LanguageSettingsTest`'s `@AfterEach` hardcoded English, which is the right answer on a runner whose default locale is English and the wrong one on a machine whose default is Persian: there, `GameLanguageTest.startsInTheLanguageTheDeviceIsSetTo` would find English left behind by a class that ran before it. Captured in `@BeforeEach`, restored in `@AfterEach` | `a17f010` |
| 2026-09-17 | 95 | R7.3 | the four screens the tables were written for now read from them, and the second copy of every word is gone: the menu, the pause overlay, the run summary and the HUD drew the literals their table entries were copied from, so a player who switched to Persian got a Persian settings screen inside an English game. Three entries were reshaped rather than used as written, because the tables had been drafted from the literals without the draw calls in front of them -- `PauseStrings.WAVE_VALUE` took one argument for a line that draws two numbers and `COIN_VALUE` beside it was read by nothing; `GameOverStrings` held `ASCEND` plus `HEARTWOOD_TOTAL` and `TIER` plus `ASCEND_SUBTITLE` for one drawn string each; `HudStrings` carried `CHARGE`, `CRIT_CHANCE`, `XP`, `GROVE` and `COIN_TOTAL`, none of which the HUD draws. Two decisions were forced by measurement rather than taste: U+2190 is in neither shipped face (checked with `fontTools` against all four TTFs) and an ASCII `->` inside a Persian sentence is mirrored into `-<` by the bidi pass, so `TIER_PROGRESS` says "ردهٔ ۳ به ۴" and keeps the arrow only on the English side; `SPEED` keeps the HUD's "x" in English and takes U+00D7 in Persian, because `TranslationTableTest` refuses a Latin letter in a Persian value. `pause.waveLabel`'s `.toUpperCase(Locale.ROOT)` is gone with the import: upper-casing is a property of the English entry now. Every number goes through `GameLocale.number` or `.percent`, so the Persian side gets Persian digits and no sentence mixes two numbering systems. English is byte-identical to what these screens drew before, which is what `MainMenuRendererTest`'s "$ 417" and "$ 0" and `PremiumMainMenuHudContractTest`'s "25 / 75 XP" and "MAX" passing unchanged actually proves. Core 191/805, `ciStaticAnalysis` green, both CI jobs green | `1a3b379` |
| 2026-09-17 | 95 | R7.3 | the gate the item asks for: a build now fails when a renderer draws its own words. `TranslationTableTest` is the untranslated half and alone proves nothing, because a screen can ignore a perfect table and draw the literal it was copied from. `DrawnStringProvenanceTest` is the other half, and its scope is the render package because `OverlayText` is package-private, so nothing outside it can put a string on screen. Which literals are user-facing is decided by where they sit rather than by a search for English, because a renderer holds diagnostics too and "Expected 12 frames for stonekin, found 9" is read by a developer: a literal counts when it is in the arguments of a `draw*` call (which covers `drawRow` and `drawMenuAction`, the helpers a check on `text.draw` alone would miss), in the body of a method whose name holds label/text/title/caption/hint/summary/name/line in either case anywhere in the name (`experienceLabel` and `labelFor` are the same kind of method and this codebase spells them both ways), or in a `String.format`/`join` call -- and inside those spans it still has to look like a sentence once its format specifiers are removed, which is what stopped `ResidencyReport`'s "%.1f" being reported as untranslated. Comments are blanked before the scan and blanked rather than cut, so a javadoc quoting a screen's words is not mistaken for a call drawing them and the offsets in a report still line up. Nine renderers predate the tables and are named in `NOT_YET_MIGRATED`, and `nothingIsListedThatHasAlreadyMoved` fails when one stops offending, so the list can only shrink; the five screens that speak through tables are held to zero. Two of the three shapes were found by running it: `StageGrade`'s "DAWN 1-50" looks user-facing and is read only by tools, and `FloatingCoinTextRenderer`'s "$ +" is drawn but sits in `labelFor`. Core 193 classes / 813 tests / 0 failures, `ciStaticAnalysis` green after ten PMD findings and two SpotBugs findings in the test itself were fixed rather than excluded | `f485b23` |
| 2026-09-17 | 95 | R7.3 | the screen mirrors, and the tap target mirrors with it. Shaping makes a Persian sentence read correctly inside the box it is drawn in; it does not move the box, so a settings row whose title sat 36 units in from the left edge still sat 36 units in from the left edge in Persian -- legible and backwards at the same time. `render/UiMirror` is the function the absolute numbers now go through, and its vocabulary is leading and trailing rather than left and right because that is the only pair that means the same thing in both languages. `OverlayText.drawLeading`/`drawTrailing` measure the run shaped before mirroring it, because the advance of a joined Persian word is not the sum of its letters' advances and mirroring with the unshaped width would leave the last word hanging off the edge it was measured against. Every element on the settings screen is placed by an inset and a container now -- the five rows' titles lead, their values trail, the note panel's three lines lead, the header icon leads and the close button trails -- and the numbers that were there (136f, 590f, 156f, 130f, 60f, 588f) are still true in English, which is what makes this a mirroring and not a re-layout. The close button is why `UiMirror` is public: it is the one asymmetric element on the screen, so it is the one that moves, and a mirrored drawing with an unmirrored hit test is a button the player can see in one place and press in another. `SettingsTouchLayout.closeX()` calls the same `trailingOnScreen` the renderer calls for the icon, with the inset the box has always had (720f - 570f - 100f = 50f), and `input` already depended on `render` so no new direction enters the graph. The five rows span 100f..620f of a 720f screen, their margins are equal, and the touch test now asserts in Persian rather than assuming that mirroring cannot move their taps. `UiMirrorTest` holds the arithmetic in both directions. Core 193/813, `ciStaticAnalysis` green | `e0b0146` |
| 2026-09-17 | 95 | R7.3 | the menu, the pause overlay and the run summary mirror too, and every element on all three is placed by an inset through `UiMirror` rather than by a number added up by hand: the menu's six row icons and their twelve lines, its coin panel and the badge inside it, the pause overlay's header, four destination rows, resume row and two bottom readouts, and the run summary's five measurement rows and three buttons. In English every x is unchanged, which is what makes it a mirroring and not a re-layout, and it is why no presentation test moved. Two decisions the settings screen did not need. Mirroring about the screen is only correct inside a container that is itself centred on it, and the menu's coin panel spans 500f..676f, so `UiMirror.centre()` mirrors a centred element about its own container: the panel goes through `leadingOnScreen`, its icon through `leading` with the 8f inset it has always had (508f - 500f) and its count through `centre`. And `MainMenuRenderer.drawShadowed` changed meaning rather than signature -- both its callers are a menu row's title and subtitle, so it draws leading now and its parameter is named `inset`, because a wrapper that kept drawing at an absolute x would be the one place a future row could quietly opt out. None of the three needed its touch layout touched, and that was checked rather than assumed: the menu's rows, the pause overlay's and all three game-over buttons are `BUTTON_X` 120f over 480f, and 2 * 120f + 480f is 720f, so every box is its own mirror image. Core 193 classes / 813 tests / 0 failures, `ciStaticAnalysis` green, both CI jobs green on `bd9d4a5` | `bd9d4a5` |
| 2026-09-18 | 95 | Roadmap | the three screens that mirror are logged, and R7.3's remainder is written down as the HUD plus the ratcheted renderers, in the order they have to happen | `90b9b6a` |
| 2026-09-18 | 95 | R7.3 | the hard-coded-string gate stops being a rule about the render package and becomes a rule about the game: it walks all 257 main sources, adds a content rule for enum constructor arguments (which is how `SkillEvolution`'s 28 lines and `Trophy`'s 24 became visible at all), excludes identifiers and the `balance/` package that writes `docs/BALANCE.md` under its own gate, and reports **320 untranslated strings in 27 files** where it had been reporting nine renderers. `NOT_YET_MIGRATED` holds all 27 and stays a ratchet in both directions. Nothing was migrated by this commit and nothing was excluded to make a number smaller; the count went up because the ruler got longer | `b1b5d41` |
| 2026-09-18 | 95 | R7.3 | `i18n/RunStrings`: the four omens' names and the line under each saying what it does to you, plus the two run lengths, out of `model/WaveModifier` and `model/GameMode` and into a table -- the multipliers are balance and stayed in the model, the words are a language. `NONE` keeps an empty label because the absence of an omen has no words, and a save still stores `name()`, so a language change cannot orphan a run. The details are sentences on both sides ("heavier blows" / "ضربه‌هایشان سنگین‌تر است") because the HUD draws label and detail on one line at 0.44 scale and a Persian fragment reads as a typo. The ratchet caught the two files that stopped offending and refused to build until their names came off: 27 to 25 | `73ce75a` |
| 2026-09-18 | 95 | R7.3 | `i18n/OnboardingStrings` carries the first vigil's twelve strings, which is what `OnboardingStep`'s own javadoc had promised the locale work would do: the action and the budget stay in the enum as gameplay data, the coached line and the look-here hint move to a table. The banner's three left-anchored draws became `drawLeading` against the band as their container, because a Persian line at 1.06 scale is wider per word than its English equivalent and a left-anchored one could have run past the band's edge. The shop hint names no side in either language: the English said "bottom left" of a button at x=375 of 720, and a named side would have to be reworded again once the HUD mirrors. 25 to 23 | `fcf4347` |
| 2026-09-18 | 95 | R7.3 | `i18n/ItemStrings`: the anvil's five verdicts, the two equipment sets, the Inventory status line, an item's "+N" forge suffix and the golden label that rises off a kill. The point is the numbers rather than the vocabulary -- "$ +30", "Verdant Covenant 2/4" and "Starfall Bow +1" were all built by concatenating an `int`, which puts Latin digits inside a Persian sentence, and every count now goes through `GameLocale.number`. The kill label follows `MenuStrings.COINS`' established split ("$ %1$s" against "%1$s سکه"), and written "+%1$s سکه" it displays as "سکه ۳۰+" because Persian digits are AN in the bidi pass and their run reverses inside the RTL paragraph -- the same result `GameNumbers.signed` documents, reached through a pattern instead of a second code path. The gate then caught two letterless leftovers that were both real: the " +" in `displayName` was a number concatenated without a language, and the " \|" in the status line was layout written in one language only, so the line became one pattern with a slot per set and `EquipmentSetBonusTest.theStatusLineHasOneSlotPerSet` fails the build if a third set arrives without a third placeholder. 23 to 20 | `8715cbe` |
| 2026-09-18 | 95 | R7.3 | the Root Network screen reads from `i18n/RootNetworkStrings` -- eight entries, keeping the terms the other tables already set ("شبکه ریشه", "چوب دل") because a currency that changes name between screens is one a player cannot plan with -- and its close box becomes the screen's one mirrored element: `RootNetworkTouchLayout.closeX()` is the same `UiMirror.leadingOnScreen` the renderer calls for the box and the word centred in it, and `closeAt()` hit-tests against it, which `RootNetworkTouchControllerTest` now asserts in both languages including that the old position is dead. The nodes do not move: their coordinates are the shape of the tree, they carry icons rather than words, and the only text on one is centred. `AWAKENED` is the one entry whose sides are not the same word -- the English "OK" is a tick mark, and in Persian that reads as agreement rather than as a state. What this does not localise is recorded in the item above rather than left to be found: the 23 node names in `RootNetworkCatalog` and every item name in `EquipmentCatalog`, neither of which the gate can see. 20 to 19 | `dca2e4b` |
| 2026-09-18 | 95 | R7.3 | the Convergence draft is Persian end to end: `i18n/TrialStrings` holds 47 entries, the screen's seven words and the thirteen trials' three lines each plus what a locked card waits for, and `trials/TrialId` keeps its modifiers, unlock conditions and icon keys while `name()` stays what a save stores. Both halves in one commit because they are one sentence to a player -- a card reads as a name over a green reward over a red risk, and a Persian frame around three English lines would have left the one screen where the choice is made unreadable. Percentages take U+066A with Persian digits, which is what `GameNumbers.percent` produces for a computed value, and no entry carries U+0654: `arabic_reshaper`'s default deletes it, four of the 245 golden vectors prove it is gone before the shaper finishes, and a mark that cannot survive the pipeline only misleads a reader of the source. `TranslationTableTest` caught Swift Hollow and Miser's Pact both paying "+30% coin income" written twice; they share one entry now. The card lines became `drawLeading`/`drawTrailing` against the card, and its panels are 600f at x=60f of 720f, which was checked rather than assumed and is why no touch layout moved. 19 to 17 | `bf3776e` |
| 2026-09-18 | 97 | Phase 97 | **the re-audit, and the gate is still not met: 777 of 920.** `docs/audit/AUDIT_2026-09-18.md` scores `bf3776e` with the same ten categories, the same weights and the same rule -- a sub-item is credited only when the artefact exists and something would notice if it stopped being true -- and its figures come from `tools/audit/measure_round.py` on this checkout, frozen as `docs/audit/MEASUREMENTS_2026-09-18.json` (193 classes / 816 tests / 0 failures / 27.9 s; 16 KTX containers beside 157 PNGs; nine filed perf runs; APK 20,872,599 B against the 26,489,934 B the last audit measured). Movement since `12e4dc0` is +46 and is almost entirely two things: category 7 from 40 to 69 (the Back button 0 to 10, Persian and RTL 0 to 15, onboarding and tooltips +3) and category 8 from 66 to 74, which is the last audit's own §9 estimate of what shipping containers would do, now measured; categories 1, 3, 4 and 6 did not move because nothing behind them did. It also found a documentation defect rather than inheriting one: the 2026-09-17 audit's rows sum to 731 while this file and the README both said 722, so the DoD above is corrected, the old log row keeps its number with the correction appended, and the re-audit scores the miss in its category 10. R7.5 (accessibility) and R7.6 (store assets) were attempted in planning and not started: R7.6 needs a device for its screenshots and there is no Android SDK in this sandbox, and R7.5's three parts are a settings row, a second rarity channel and a contrast measurement, none of which is honest to claim half-built. Phase 95 stays `[~]` | this push |

| 2026-09-18 | 95 | R7.3 | `i18n/StoryStrings`: the game's own voice is now in both languages -- the four first-encounter boss title cards, the five beats of the planting ceremony, the six wave-milestone reflections and the trophy line -- and the switches that decide which one plays stay in `story/` and `progression/`, because when a line plays is presentation logic and only the words moved. They are one table rather than four because they share a narrator, and a narrator whose voice differs between screens is one the player notices. The English side is byte-identical to the literals it replaces, which is what `BossTitleCardsTest`, `CeremonyLinesTest`, `ReflectionLinesTest` and `TrophyPresenterTest` passing unchanged actually proves. The em dash stays on both sides rather than becoming the "  \|  " the panel tables use, and that was measured rather than assumed: `fontTools` against the shipped Vazirmatn-Bold face finds U+2014, U+066A and U+200C present. The trophy line's join word came out of the concatenation and into a pattern, so a language that would order the line differently can; what it still cannot reach is `Trophy`'s own 24 names, which is recorded in the table's javadoc and is the reason that file is still on the ratchet. 17 to 13 | this push |
## Definition of done
