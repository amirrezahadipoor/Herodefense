...
def runtime_entry(master_asset: dict, runtime_clips: dict, runtime_page: tuple[int, int]) -> dict:
    ...
    entry["masterRender"] = {
        ...
        "engineVersion": master_asset.get("engineVersion"),
...
def publish(master_dir: Path, runtime_dir: Path, reviewed_manifest_path: Path) -> dict:
    ...
    for master_asset in master_manifest["assets"]:
        ...
        entry = runtime_entry(master_asset, clips, runtime_page)
        for field in ("clips", "frameSize", "sheetWidth", "sheetHeight", "sheets", "renderSupersample",
                      "renderSamples", "visualQuality", "masterRender"):
